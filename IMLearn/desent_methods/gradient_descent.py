...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        sum, iterations, weight = 0, 1, f.weights
        minimum = f.compute_output(X=X, y=y), weight
        sum += weight.copy()
        for index in range(1, self.max_iter_ + 1):
            iterations = index
            previous = weight # save previous weight
            weight = weight - self.learning_rate_.lr_step(t=index) * f.compute_jacobian(X=X, y=y)
            f.weights = weight
            sum += weight.copy() # adds the sum of the weights at each iteration
            if f.compute_output(X=X, y=y) < minimum[0]:
                minimum = f.compute_output(X=X, y=y), weight
            diff = weight - previous
            delta = np.sqrt(np.matmul(diff, diff)) # was told to do this in latest update
            self.callback_(self,weights=weight, val=f.compute_output(X=X, y=y),  grad=f.compute_jacobian(X=X, y=y),   t=index,
                           eta=self.learning_rate_.lr_step(t=index), delta=delta)

            if delta < self.tol_:
                break
        avg = sum/iterations
        return [weight, minimum[1], avg][OUTPUT_VECTOR_TYPE.index(self.out_type_)]