...
class LogisticModule(BaseModule):
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        X_w = np.matmul(X, self.weights)
        return np.matmul((1 / y.size) * np.transpose(X), ((1 / (1 + np.exp(-X_w))) - y))
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        self.regularization_module_.weights = self._weights[1:].copy()
        return self.fidelity_module_.compute_jacobian(X=X, y=y) + self.lam_ * (np.append(np.array([0]), self.regularization_module_.compute_jacobian(X=X, y=y)))
